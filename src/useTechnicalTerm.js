...
export const useTechnicalTerm = () => {
  const [technicalTerm, setTechnicalTerm] = useState([
    ...
    },
    {
      term: ['オレキシン'],
      explanation:
        '摂食・覚醒調節作用を有する神経ペプチド。視床下部外側部のニューロンのみが産生し、中枢神経全域へ投射される。覚醒時にオレキシン神経活動は活発になり、活動が抑制されるとノンレム睡眠へと移行する。オレキシンニューロンは睡眠中枢から抑制され、覚醒中枢を刺激する（睡眠・覚醒のスイッチflip-flop仮説）。オレキシンやその受容体が欠損するとナルコレプシーという睡眠障害を伴う。また摂食行動促進作用も存在する。',
    },
    {
      term: [
        'ナルコレプシー',
    ...
    },
    {
      term: ['Papez回路'],
      explanation:
        '大脳辺縁系2大神経構造。支障前核群を通る。海馬体→脳弓→乳頭体→支障前核群→帯状回後部→海馬傍回後部',
    },
    {
      term: ['延髄最後野'],
    ...
    {
      term: ['摂食中枢', '視床下部外側野', '室傍核'],
      explanation:
        '摂食中枢は視床下部外側野LHに存在し、室傍核などと連携して摂食活動を促進する。。グルコース濃度↑で抑制され、遊離脂肪酸濃度↑で発火促進され、視索前野の加温で発火抑制、加冷で発火促進されるなど、緻密に制御されている。',
    },
    {
      term: ['満腹中枢', 'VMH', '視床下部腹内側核', '弓状核'],
      explanation:
        '満腹中枢は視床下部腹内側核VMHに存在して、弓状核などと連携して摂食活動を抑制する。。グルコース濃度↑で発火促進、遊離脂肪酸濃度↑で抑制され、視索前野の加温で発火促進、加冷で発火抑制されるなど、緻密に制御されている。',
    },
    {
    ...
    },
    {
      ...
      explanation:
        '脂肪細胞から分泌される146アミノ酸残基からなる蛋白。レプチンあるいはレプチン受容体が欠損する家系では著しい肥満症を呈する。レプチンが体脂肪率を反映して血中濃度が上昇すると、視床下部（脂肪組織-レプチン-視床下部系）の受容体へ伝達され、摂食抑制（味細胞での甘味感受性の低下など）・交感神経活動亢進・エネルギー消費亢進を行い体脂肪量を一定に保つフィードバックを形成する。アディポネクチンとほぼ同じ視床下部弓状核に受容体が存在するが、作用は正反対である。',
    },
    {
    ...
    },
    {
      term: ['グレリン', 'ghrelin'],
      explanation:
        '視床下部弓状核や胃・腸（空腹時）で分泌される。グレリンはペプチド部分が脂肪酸と結合してないと食欲刺激活性を示さない。摂食行動の生理的信号物質で、成長ホルモン分泌亢進・摂食行動亢進・胃酸分泌刺激・脂肪の蓄積・心機能の改善の作用がある。弓状核NPYニューロンに受容体があり、インスリンと相反する作用を示す。',
    },
    {
      term: ['インスリン', 'insulin'],
      explanation:
