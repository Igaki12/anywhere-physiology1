...
export const useTechnicalTerm = () => {
  const [technicalTerm, setTechnicalTerm] = useState([
    ...
    {
      term: ['ポリモーダル受容器', 'polymodal nociceptor'],
      explanation:
        '伝導速度のもっとも遅い無髄のC神経線維の末梢端（侵害受容器）。強い機械刺激・化学物質刺激・温度刺激（高温・低温）を受容し活性化。',
    },
    {
      term: ['METチャネル複合体', 'mechano-electrical transducer channel'],
      explanation:
        '物理的な伸展刺激に対して開口するK⁺イオンチャネル。有毛細胞が適方向（基底小体の方向）に屈曲すると、チャネルが開きK⁺が高濃度の内リンパ液から細胞内に流入し脱分極が生じる。逆方向に屈曲すると、チャネルが閉じK⁺の流入が止まり過分極となる（感覚毛を動かさなくても機械受容器チャネルは開いている）',
    },
    // {
    //   term:[''],
    //   explanation:'',
